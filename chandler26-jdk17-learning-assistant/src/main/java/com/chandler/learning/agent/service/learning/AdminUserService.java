...
import com.chandler.learning.agent.domain.dto.learning.AdminUserPageResponse;
import com.chandler.learning.agent.domain.dto.learning.AdminUserResetPasswordRequest;
import com.chandler.learning.agent.domain.dto.learning.AdminUserResponse;
import com.chandler.learning.agent.domain.dto.learning.AdminUserSaveRequest;
import com.chandler.learning.agent.domain.dto.learning.AdminUserUpdateRequest;
...
public class AdminUserService {
    ...
    public AdminUserResponse create(LearningUser operator, AdminUserSaveRequest request) {
        ...
        LearningWordbook defaultWordbook = LearningWordbook.createDefault(user.getId(), now);
        defaultWordbook.setCreateBy(operator.getId());
        defaultWordbook.setUpdateBy(operator.getId());
        wordbookMapper.insert(defaultWordbook);
    ...
    private List<AdminUserResponse> toResponses(List<LearningUser> users) {
        ...
        Map<Long, Integer> wordbookCounts = collectCounts(wordbookMapper.selectMaps(new QueryWrapper<LearningWordbook>()
                .select("user_id AS userId", "COUNT(*) AS count")
                .in("user_id", userIds)
                .eq("deleted", false)
                .groupBy("user_id")));
        Map<Long, Integer> planCounts = collectCounts(planMapper.selectMaps(new QueryWrapper<LearningPlan>()
                .select("user_id AS userId", "COUNT(*) AS count")
                .in("user_id", userIds)
                .eq("deleted", false)
                .groupBy("user_id")));
    ...
    private Map<Long, Integer> collectCounts(List<Map<String, Object>> rows) {
        Map<Long, Integer> result = new HashMap<>();
        for (Map<String, Object> row : rows) {
            Long userId = toLong(row.get("userId"));
            if (userId != null) {
                result.put(userId, toLong(row.get("count")).intValue());
            }
        }
        return result;
    }
