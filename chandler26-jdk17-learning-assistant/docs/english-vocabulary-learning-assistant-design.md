...
- 个人信息：
  ...
  - 词书数量、单词数量、待复习数量。
  - 单词表管理：采用“列表 + 新增按钮 + 弹窗”的管理方式，支持新建/编辑词书、设置默认词书、单个删除词书，不提供清空按钮。
  - Agent 管理：模型列表、学习 Agent、Prompt 模板、默认发音、强制刷新。
  - 模型列表：新增/编辑模型、配置 API Key 和 Base URL、启停、默认模型、优先级、单个删除；新增/编辑通过弹窗完成，供应商与模型明细联动，顺序使用数字输入，不提供清空按钮。
...
- `个人信息 / Agent 管理`：模型配置采用列表和新增/编辑弹窗，支持单个删除；不提供清空按钮。
- `单词本`：按词表查看单词、筛选状态、通过弹窗修改状态、红色删除图标删除词条、查看学习卡片式详情和编辑笔记。
...
- 左侧列表标题为“单词列表”，只展示词条、掌握度、下次复习时间和少量操作。
- 标签信息不在列表中铺开，详情中通过“查看标签”按钮展开。
- 熟练程度通过弹窗选择 `熟悉`、`模糊`、`遗忘`。
- 删除词条按钮使用红色删除图标。
- 详情区域占主要宽度，展示学习卡片内容并复用学习/复习同一份 Markdown 笔记。
