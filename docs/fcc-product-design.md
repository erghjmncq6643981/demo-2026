...
核对日期：2026-09-19。范围为 Java 21 FCC、管理端、坐席端、Go Sidecar 与 FreeSWITCH 运维端；Java 17 FCC 仅作为前代验证参考。产品为全新系统，无旧业务兼容要求。本文区分源码现状与待实现验收目标，不以页面、DDL 或接口存在认定能力可用。
...
| 流程定义 | JSON 编辑、版本、草稿、发布、ROUTE 阶段编译 | 定义 Schema、完整节点执行、发布激活确认、版本固定、仿真 |
| 话单与录音 | 分页、统计、详情、录音元数据与读取入口 | 最终事实核对、录音完成/缺失状态、授权与审计联调 |
...
| P0 | 双向通话最小闭环 | 至少覆盖呼入/外呼、接听/拒接、双方挂机、忙线/无应答、双向音频和对应 CDR；保留命令/事件/Call/Leg 关联证据 |
| P0 | 事件可靠性与恢复 | 稳定源 eventId、重复/乱序保护、终态幂等；断网/重启后恢复活跃 Call、坐席占用与录音事实；耐久事件方案需明确 ACK/重放策略 |
...
| P1 | 完整 IVR | 播放、收号、分支、超时/错误出口、转人工、挂机；共用定义校验和执行语义，仿真不触发生产话务 |
...
自动外呼是 fcc-server 核心能力，按实施清单在基础通话可靠后交付，不再归为 P2 扩展。基础客户资料与客户来电关联随 Windows 弹屏交付；外部 CRM/工单集成、质量评价与检索、报表导出、预测式外呼和可视化流程编辑属于后续扩展。可视化编辑应建立在稳定定义模型和完整执行器上。

前端现有超过 600 行的组/CDR/分机视图继续按列表、详情、表单拆分；完善错误、长文本、移动敏感宽度与登录失效验证。分包优化、契约测试和生成产物治理与功能同步推进，不以行数降低替代正确性。

## 7. 部署与验证约束

- 编译需要 JDK 21，本轮用临时 JDK 已通过 Java 编译；Go 以 go.mod 声明为准。仍需在 CI 和完整环境验证核心业务。
...
- 新环境采用空库基线；已有开发环境先备份，再审查迁移和执行顺序，不自动删除已有数据，不导入演示坐席/话单。
...
2026-09-20 已增加验证码绑定、固定呼入/坐席先接听呼出、客户资料、通知/渐进式调度及 Electron 测试安装包。MySQL 与 JetStream 已有本机真实服务验证，FreeSWITCH 媒体及 Windows 交互尚未验收。Java 全量测试命令通过：54 项通过，2 项依赖真实坐席的 WebSocket 用例跳过；不等于真实媒体验收通过。当前实现、部署步骤及剩余阻断以 [跨电脑验收记录](fcc-cross-machine-acceptance.md) 为准。
