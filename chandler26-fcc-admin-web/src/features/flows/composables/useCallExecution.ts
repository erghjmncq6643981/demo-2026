...
interface ExecutionResponse {
  instance: { id?: string; snapshot?: string; status?: string };
  steps: StageExecution[];
  nextCursor: string;
}
/** 分页读取真实执行事实；关闭弹窗后失效旧请求，不模拟进度。 */
export function useCallExecution(callId: Ref<string>) {
  const flow = ref<StagedFlow | null>(null),
    executions = ref<StageExecution[]>([]);
  const error = ref(''),
    loading = ref(false),
    cursor = ref(''),
    status = ref(''),
  ...
  async function load(more = false) {
    ...
    try {
      const data = await apiClient.get<unknown, ExecutionResponse>(
        `/flow-studio/calls/${encodeURIComponent(callId.value)}`,
        { params: { after: more ? cursor.value : '0' } },
      );
      if (current !== generation) return;
      if (data.instance.snapshot) {
        const snapshot = JSON.parse(data.instance.snapshot);
        flow.value = snapshot.definition;
        version.value = `${snapshot.flowKey} · v${snapshot.versionNo}`;
      }
      status.value = data.instance.status || '';
      executions.value = more ? [...executions.value, ...data.steps] : data.steps;
