...
<template>
  <div class="flex-1 flex flex-col gap-6 overflow-y-auto pr-1">
      ...
      >
        <div>
          <div class="flex items-center gap-2">
            ...
            <span
              class="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 text-[11px] font-bold border border-emerald-100"
            >
              真实数据库直连
            </span>
          </div>
          <p class="text-xs text-slate-400 mt-0.5">
            全网各通道实时接通效能数据报表（已清空历史 Mock 假数据）
          </p>
        </div>
        <div class="flex items-center gap-3 text-xs">
          <span
            >统计周期:
            <strong class="text-slate-800 font-bold font-mono"
              >{{ todayStr }} ~ {{ todayStr }}</strong
            ></span
          >
          <button
            @click="handleRefresh"
    ...
    <!-- 通话趋势与类型分析 -->
    <div
      class="bg-white rounded-3xl border border-slate-100 shadow-card p-6 flex-1 flex flex-col justify-between min-h-[320px]"
    >
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-2 text-xs">
            ...
            <select
              ...
              class="bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 text-slate-800 font-bold focus:outline-none cursor-pointer"
            >
      ...
      <!-- 趋势展示区 (实测空状态与实时绘制) -->
      <div
        class="h-60 bg-slate-50/50 rounded-2xl border border-slate-100 p-4 relative flex flex-col justify-between overflow-hidden"
      >
        <!-- 如果暂无测试通话流水，显示清爽直观的待测空状态 -->
        <div
          v-if="currentStats.count === 0"
        ...
        >
          ...
          <p class="text-[11px] text-slate-400">
            系统已接入真实话单库，产生通话后将在此绘制实时量级与接通走势波形
          </p>
        </div>

        <!-- Y轴基准线 -->
        <div
          class="absolute inset-x-4 top-1/4 border-b border-dashed border-slate-200 text-xs text-slate-400 pl-1 font-mono"
        >
          10
        </div>
        <div
          class="absolute inset-x-4 top-2/4 border-b border-dashed border-slate-200 text-xs text-slate-400 pl-1 font-mono"
        >
          5
        </div>
        <div
          class="absolute inset-x-4 top-3/4 border-b border-dashed border-slate-200 text-xs text-slate-400 pl-1 font-mono"
        >
          1
        </div>
        ...
        <!-- X 轴时间标签 -->
        <div
          class="w-full flex justify-between text-xs font-mono text-slate-400 pt-2 z-10 mt-auto"
        >
