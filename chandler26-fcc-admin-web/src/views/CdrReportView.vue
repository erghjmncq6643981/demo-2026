...
<template>
  <div class="h-full flex-1 flex flex-col gap-6 overflow-y-auto pr-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 shrink-0">
      <!-- 指标 1 -->
      <div
        class="bg-white p-6 rounded-3xl border border-slate-100 shadow-card flex flex-col justify-between hover:shadow-card-hover transition-all"
      >
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-bold text-slate-500">今日呼叫总数</span>
          <div
            class="w-9 h-9 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center text-sm font-black"
          >
            ↗
          </div>
        </div>
      ...
      >
        <div class="flex items-center justify-between mb-2">
          ...
          <div
            class="w-9 h-9 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center text-sm font-black"
          >
            ↩
          </div>
        </div>
      ...
      >
        <div class="flex items-center justify-between mb-2">
          ...
          <div
            class="w-9 h-9 rounded-2xl bg-purple-50 text-purple-600 flex items-center justify-center text-sm font-black"
          >
            ⏱
          </div>
        </div>
    ...
    >
      ...
      >
          ...
          <button
            ...
            :class="
              activeSubTab === 'records'
                ? 'font-black text-slate-900 text-lg border-b-2 border-brand-500'
                : 'font-bold text-slate-400 hover:text-slate-700 text-base'
            "
          >
            <span>📞 通话记录</span>
            <span
        ...
        <div class="flex items-center gap-3 text-xs">
          <div
            class="flex items-center gap-2 bg-slate-50 border border-slate-200/80 px-3.5 py-1.5 rounded-full text-slate-700 font-semibold"
          >
            <span>📅</span>
      ...
      >
        ...
        <div class="flex-1 overflow-x-auto overflow-y-auto">
          <table class="w-full text-sm text-left">
            <thead
              class="text-slate-400 border-b border-slate-100 pb-2 text-xs font-bold uppercase sticky top-0 bg-white z-10"
            >
              <tr>
              ...
              <tr v-if="callRecords.length === 0">
                <td colspan="11" class="py-16 text-center">
                  <div class="flex flex-col items-center justify-center">
                    <div
                      class="w-14 h-14 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center text-2xl mb-3 shadow-xs"
                    >
                      📞
                    </div>
              ...
              >
                ...
                <td class="py-3.5 px-3.5">
                  <div class="flex items-center gap-2.5">
                    <div
                      class="w-8 h-8 rounded-full bg-blue-50 text-brand-600 font-black flex items-center justify-center text-xs shrink-0 border border-indigo-100"
                    >
                ...
                <td class="py-3.5 px-3">
                  <span
                    class="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 text-xs font-bold font-sans"
                  >
                    {{ item.carrier }}
                  </span>
                </td>
                ...
                <td class="py-3.5 px-2.5">
                  <span
                    class="font-mono text-xs text-slate-600 font-bold bg-slate-50 px-2 py-0.5 rounded border border-slate-200"
                  >
                    {{ item.agentWorkNo }}
                  </span>
                </td>
                ...
                <td class="py-3.5 px-2.5">
                  <span
                    ...
                    class="text-amber-600 font-black text-xs flex items-center gap-1"
                  >
                    <span class="text-sm">↗</span><span>呼出</span>
                  </span>
                  <span
                    v-else-if="item.direction === 'INBOUND'"
                    class="text-blue-600 font-black text-xs flex items-center gap-1"
                  >
                ...
                <td class="py-3.5 px-2.5">
                  <span
                    ...
                    class="px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 font-bold text-xs"
                  >
                    已接通
                  </span>
                  <span
                    v-else-if="item.status === 'MISSED'"
                    class="px-2.5 py-1 rounded-full bg-rose-50 text-rose-600 font-bold text-xs"
                  >
                    未接听
                  </span>
                  <span
                    v-else-if="item.status === 'BUSY'"
                    class="px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 font-bold text-xs"
                  >
                    坐席忙
                  </span>
                  <span
            ...
            <tbody class="divide-y divide-slate-50">
              ...
              <tr v-if="filteredCallbackRecords.length === 0">
                <td colspan="9" class="py-16 text-center">
                  <div class="flex flex-col items-center justify-center">
                    <div
                      class="w-14 h-14 rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center text-2xl mb-3 shadow-xs"
                    >
                      ⏳
                    </div>
                    <p class="text-sm font-bold text-slate-700 mb-1">
                      数据库已进入真实测试模式，暂无未接待回拨任务
                    </p>
                    <p class="text-xs text-slate-400 max-w-sm leading-relaxed">
                      当有客户呼入因排队超时、坐席全忙或放弃未接听时，系统将自动产生真实待回拨工单并在此呈现
                    </p>
                  </div>
                </td>
              </tr>
              <tr
                v-for="item in pagedCallbackRecords"
                :key="item.id"
                class="hover:bg-slate-50/80 transition-colors"
              >
                <!-- 1. 客户号码 -->
                ...
                <td class="py-4 px-3">
                  ...
                  <span
                    ...
                    class="px-2.5 py-1 rounded-full bg-indigo-50 text-brand-600 font-bold text-xs"
                  >
                    已派单 ({{ item.assignee }})
                  </span>
                  <span
    ...
    <div
      v-if="playingAudio"
      class="fixed bottom-8 inset-x-0 mx-auto max-w-2xl bg-white rounded-full shadow-popover border border-slate-200 p-3 px-6 flex items-center justify-between z-50 animate-fadeIn"
    >
      <div class="flex items-center gap-3.5">
        <div
          class="w-10 h-10 rounded-full bg-brand-500 text-white flex items-center justify-center shadow-pill text-sm font-bold"
        >
          🎧
        </div>
        <div>
          ...
          <div class="text-xs text-slate-400 font-mono font-bold mt-0.5">
            总时长: {{ currentAudioInfo?.duration }}
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          @click="closeAudio"
          class="text-slate-400 hover:text-slate-600 font-bold text-sm cursor-pointer px-2 py-1"
        >
          ✕
        </button>
      </div>
    ...
    <div
      v-if="showDetailModal && selectedCdr"
    ...
    >
      ...
      >
            ...
            <div>
              <div class="flex items-center gap-2">
                <h2 class="text-base font-black text-slate-900">
                  通话全生命周期过程详情追踪 (Call Journey Trace)
                </h2>
                <span
        ...
        >
          ...
          <button
            @click="showDetailModal = false"
            class="px-6 py-2 bg-[#1677ff] hover:bg-blue-600 text-white text-xs font-bold rounded-xl cursor-pointer shadow-xs transition"
          >
            关闭
          </button>
        </div>
