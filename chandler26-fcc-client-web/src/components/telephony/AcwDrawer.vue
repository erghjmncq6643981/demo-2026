<template>
  ...
  <div
    ...
    class="fixed inset-y-0 right-0 w-96 bg-white shadow-2xl border-l border-slate-100 z-50 p-6 flex flex-col justify-between animate-in slide-in-from-right duration-300"
  >
    <div>
      <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
        <div>
          <h3 class="font-extrabold text-sm text-slate-900">通话小结与工单登记</h3>
          <p class="text-xs text-slate-400 mt-0.5">ACW 话后整理阶段</p>
        </div>
        <button
          @click="callStore.closeAcw()"
...
const intentLevel = ref('HIGH');
const notes = ref('');

function submitSummary() {
  // 话单事实由后端在挂机时写入 fcc_call_session，前端只重新拉取真实数据，
  // 不再本地拼装一条"看起来像话单"的记录。
