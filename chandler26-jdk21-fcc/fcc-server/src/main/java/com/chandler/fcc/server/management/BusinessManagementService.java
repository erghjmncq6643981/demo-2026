...
public class BusinessManagementService {
    ...
    public CustomerRecord customer(String id) {
        var actor = identity.requireManagement();
        return customers.detail(actor.tenantId(), owner(mapper.customerOwner(actor.tenantId(), id)), id);
    }
    ...
    public String save(String id, String requestedOwner, CustomerRecord record) {
        ...
        String owner = id == null
            ? agent(actor.tenantId(), requestedOwner)
            : owner(mapper.customerOwner(actor.tenantId(), id));
        record.setId(id);
        return customerService.saveFor(actor.tenantId(), owner, record, id == null);
    }
    ...
    public List<Map<String, Object>> jobs(int page, String owner) {
        return mapper.jobs(identity.requireManagement().tenantId(), owner, offset(page));
    }
     ...
     * @return ID
     */
    public String createJob(String owner, String number, String mode, int attempts, String key) {
        var actor = identity.requireManagement();
        return jobs.createFor(actor.tenantId(), agent(actor.tenantId(), owner), number, mode, attempts, key);
    }
    ...
    public List<Map<String, Object>> attempts(String id) {
        long tenant = identity.requireManagement().tenantId();
        owner(mapper.jobOwner(tenant, id));
    ...
    public void control(String id, String action) {
        var actor = identity.requireManagement();
        String owner = owner(mapper.jobOwner(actor.tenantId(), id));
        if (jobMapper.control(actor.tenantId(), owner, id, action) != 1) throw new ResponseStatusException(
            HttpStatus.CONFLICT,
            "当前状态不允许此操作"
        );
        log.info(
            "[外呼管理] 操作任务 tenantId={} operator={} jobId={} action={}",
            actor.tenantId(),
            actor.workNo(),
            id,
            action
     ...
     * @return 工号
     */
    private String agent(long tenant, String owner) {
        if (owner == null || agents.agent(tenant, owner) == null) throw new ResponseStatusException(
