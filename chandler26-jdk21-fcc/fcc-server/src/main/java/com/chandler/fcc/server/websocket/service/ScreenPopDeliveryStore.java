...
public class ScreenPopDeliveryStore {
     ...
     */
    public void save(WsMessageDTO<IncomingScreenPopDTO> message) {
        int seconds = message.getData().getRingTimeoutSeconds() == null ? 30
                : Math.max(1, Math.min(120, message.getData().getRingTimeoutSeconds()));
        try {
            mapper.save(message.getWorkNo(), message.getCallId(), json.writeValueAsString(message),
     ...
     */
    public List<String> pending(String owner) { return mapper.pending(owner); }
     ...
     * @param callId 通话标识
     */
    public void close(String owner, String callId) { mapper.close(owner, callId); }
}
