...
public class FlowExecutionRecorder {
    ...
    public void record(CallInfoBO call) {
        ...
        try {
            ...
            row.put("call",call.getCallId()); row.put("tenant",call.getData().get("tenantId"));
            if(existing==null){
                var definition=mapper.definition(call.getDataStr("flowVersionId",null),template);
                if(definition==null) throw new IllegalStateException("缺少数据库固定流程模板，请执行阶段流程迁移");
                ...
                row.put("snapshot",json.writeValueAsString(Map.of("definition",snapshot,"flowKey",definition.get("flowKey"),"versionNo",definition.get("versionNo"))));
                mapper.create(row); existing=mapper.lock(call.getCallId());
            }
            if("ENDED".equals(existing.get("status")))return;
            String step=stage(call,template);
            String token=step+":"+call.getDataStr("ivrAttempt","0")+":"+("ROUTE".equals(step)?call.getAgentChannelUuid():"");
            var context=json.readTree(existing.get("context").toString());
            if(token.equals(context.path("executionToken").asText()))return;
            row.put("step",step); row.put("token",token); row.put("id",IdUtil.nextId());
            ...
            row.put("previousStatus","END".equals(step)&&!java.util.Set.of("CONNECTED","CONFIRM").contains(previous)?"INTERRUPTED":"SUCCEEDED");
            var facts=new HashMap<String,Object>();
            for(String key:java.util.List.of("flowBranch","directOwner","groupCode","notificationConfirmed","flowSourceTime","flowCommandId"))
            ...
            row.put("input",json.writeValueAsString(facts));row.put("output",row.get("input"));
            mapper.finish(row);mapper.append(row);mapper.advance(row);
        }catch(com.fasterxml.jackson.core.JsonProcessingException e){throw new IllegalStateException("流程执行快照损坏",e);}
    }

    /** 根据权威事件及持久运行状态识别固定阶段。
     * @param call 通话 @param template 模板 @return 阶段键
     */
    private String stage(CallInfoBO call,String template){
        ...
        if(call.getStageState()==com.chandler.fcc.common.enums.CallStageState.CONNECTED)return "CONNECTED";
        if(call.getData().containsKey("bridgeRequested"))return "BRIDGE";
        if("AGENT_FIRST".equals(template))return call.getData().containsKey("agentReady")?"DIAL_CUSTOMER":"DIAL_AGENT";
