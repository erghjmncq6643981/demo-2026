...
class AgentHandshakeBoundaryTest {
    ...
    @Test void bindsOnlyAuthenticatedIdentity() {
        ...
        when(identity.authenticatePrincipal("test-token")).thenReturn(
            new AgentIdentityService.Principal("authenticated-agent")
        );
        ...
        assertEquals("test-token", attributes.get("AUTH_TOKEN"));
    }
}
