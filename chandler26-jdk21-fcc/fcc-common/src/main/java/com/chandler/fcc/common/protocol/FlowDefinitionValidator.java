...
public final class FlowDefinitionValidator {
    ...
    public static JsonNode validate(String json) {
        try {
            ...
            if (!target.path("workNo").isTextual() || !target.path("workNo").asText().matches("[A-Za-z0-9_-]{1,64}"))
                throw new IllegalArgumentException("didDirectConfig.workNo 必须为有效坐席工号，不能使用数据库 ID");
            return root;
        } catch (IllegalArgumentException e) { throw e; }
        catch (Exception e) { throw new IllegalArgumentException("流程定义不是有效 JSON 对象"); }
    }
    ...
    private static void requireFields(JsonNode node, Set<String> fields) {
        ...
        node.fieldNames().forEachRemaining(name -> {
            if (!fields.contains(name)) throw new IllegalArgumentException("流程定义包含未支持字段: " + name);
        });
    }
}
