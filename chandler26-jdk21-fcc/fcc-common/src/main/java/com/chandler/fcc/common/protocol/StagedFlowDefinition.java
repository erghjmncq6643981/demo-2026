...
public final class StagedFlowDefinition {
    ...
    private static final Map<String,List<String>> STAGES = Map.of(
        "INBOUND", List.of("ENTRY", "MENU", "BRANCH", "ROUTE", "BRIDGE", "CONNECTED", "END"),
        "AGENT_FIRST", List.of("ENTRY", "DIAL_AGENT", "DIAL_CUSTOMER", "BRIDGE", "CONNECTED", "END"),
        "NOTIFICATION", List.of("ENTRY", "DIAL_CUSTOMER", "NOTIFY", "CONFIRM", "END"));
    ...
    public static JsonNode validate(JsonNode root) {
        fields(root, Set.of("routeMode", "template", "menu", "branches", "defaultRoute", "timeoutAction", "stages"));
        if (!"INBOUND".equals(root.path("template").asText())) throw new IllegalArgumentException("仅呼入 IVR 开放参数编排");
        ...
        if (menu.path("enabled").asBoolean() && (!menu.path("prompt").isTextual()
                || !menu.path("prompt").asText().matches("/[A-Za-z0-9_./-]{1,240}\\.(wav|mp3)")
                || menu.path("prompt").asText().contains(".."))) throw new IllegalArgumentException("请选择软交换可读取的安全音频绝对路径");
        target(root.path("defaultRoute"));
        if (!Set.of("CALLBACK", "HANGUP").contains(root.path("timeoutAction").asText())) throw new IllegalArgumentException("超时动作必须为回拨待办或挂机");
        JsonNode branches = root.path("branches");
        if (!branches.isArray() || branches.size() > 10 || (menu.path("enabled").asBoolean() && branches.isEmpty()))
        ...
        for (JsonNode branch : branches) {
            ...
            if (!digit.matches("[0-9]") || !digits.add(digit)) throw new IllegalArgumentException("分支按键必须是唯一的一位数字");
            target(branch);
        }
        ObjectNode normalized = ((ObjectNode) root).deepCopy();
        var stages = JSON.valueToTree(STAGES.get("INBOUND"));
        if (root.has("stages") && !root.get("stages").equals(stages)) throw new IllegalArgumentException("阶段动作固定，不允许修改阶段目录");
    ...
    private static void target(JsonNode node) {
        ...
        if (!Set.of("AGENT", "GROUP").contains(node.path("targetType").asText())
                || !node.path("target").asText().matches("[A-Za-z0-9_-]{1,64}")) throw new IllegalArgumentException("请选择有效坐席工号或技能组代码");
        range(node.path("queueSeconds"), 5, 300, "排队时限");
    }

    /** 校验整数范围。
     * @param node 参数值 @param min 下界 @param max 上界 @param label 参数名称
     */
    ...
    /** 拒绝未知字段。
     * @param node 对象 @param allowed 支持字段
     */
    private static void fields(JsonNode node, Set<String> allowed) {
        if (!node.isObject()) throw new IllegalArgumentException("配置必须为对象");
        node.fieldNames().forEachRemaining(key -> { if (!allowed.contains(key)) throw new IllegalArgumentException("未支持字段：" + key); });
    }
}
