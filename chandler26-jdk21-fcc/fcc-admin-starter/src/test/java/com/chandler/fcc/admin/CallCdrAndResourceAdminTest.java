...
public class CallCdrAndResourceAdminTest {
    ...
    void testResourceManagement() {
        // 1. 中继线路
        Long trunkId = resourceService.createTrunk(TrunkCreateReq.builder()
        ...
        // 2. DID 号码
        Long didId = resourceService.createDidNumber(DidNumberCreateReq.builder()
        ...
        assertTrue(dids.stream().anyMatch(d -> d.getPhoneNumber().equals("01099998888")));

        // 3. 外呼号码池
        Long outId = resourceService.createOutboundNumber(OutboundNumberCreateReq.builder()
        ...
        List<OutboundNumberVO> outbounds = resourceService.listOutboundNumbers("vip");
        assertTrue(outbounds.stream().anyMatch(o -> o.getPhoneNumber().equals("02195588001")));
    }
    ...
    void testSystemConfigAndFleet() {
        // 1. 动态配置
        Long cfgId = configService.saveOrUpdateConfig(SystemConfigReq.builder()
        ...
        assertTrue(configs.stream().anyMatch(c -> "call.recording.auto-upload".equals(c.getPropName())));

        // 2. 客户端版本发布
        Long verId = fleetService.publishVersion(ClientVersionReleaseReq.builder()
                .version("1.3.0")
                .platform("WINDOWS")
        ...
        List<ClientVersionReleaseVO> versions = fleetService.listVersions("WINDOWS");
        assertTrue(versions.stream().anyMatch(v -> "1.3.0".equals(v.getVersion())));

        // 3. 坐席硬件指纹安全审计记录
        Long hwId = fleetService.recordHardware(10001L, "901001", "1.3.0",
                "00:1A:2B:3C:4D:5E", "Windows 11 Enterprise", "192.168.1.188");
        assertNotNull(hwId);

        List<ClientHardwareRecordVO> audits = fleetService.listHardwareAudit(10001L);
    ...
    void testCdrPaginationAndDetail() {
        ...
        CallSessionEntity session = CallSessionEntity.builder()
                ...
                .evaluationScore(5)
                .attributes("{\"recording_path\":\"http://storage.example.com/recordings/test.wav\"}")
        ...
        // 4. 多条件分页查询验证
        PageResult<CallCdrVO> cdrPage = cdrService.queryCdrs(CallCdrQueryReq.builder()
                .caller("13800138999")
                .agentWorkNo("901001")
                .direction("INBOUND")
                .status("COMPLETED")
        ...
        CallCdrVO found = cdrPage.getList().stream()
        ...
        assertEquals("http://storage.example.com/recordings/test.wav", found.getRecordingUrl());
