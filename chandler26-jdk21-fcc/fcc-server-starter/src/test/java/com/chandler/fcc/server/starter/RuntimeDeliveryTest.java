...
class RuntimeDeliveryTest {
 ...
 @Test void externalRouteRequiresRealConfiguration(){
  ...
  assertThrows(ResponseStatusException.class,()->policy.resolve(42,"13800000000"));
  when(mapper.outbound(42)).thenReturn(Map.of("gateway","carrier-a","caller","4000000000"));
  assertEquals("sofia/gateway/carrier-a/13800000000",policy.resolve(42,"13800000000").dialString());
  when(mapper.outbound(42)).thenReturn(Map.of("gateway","bad/route","caller","4000000000"));
  assertThrows(ResponseStatusException.class,()->policy.resolve(42,"13800000000"));
 }
 /** 所有客户查询使用认证租户与坐席；歧义匹配不选第一个。 */
 @Test void customerOwnershipAndAmbiguousMatches(){
  ...
  verify(mapper).detail(42,"alice","9007199254740993");
  when(mapper.list(42,"alice","1001",0,2)).thenReturn(List.of(new CustomerRecord(),new CustomerRecord()));
  assertTrue(service.match(42,"alice","1001").isEmpty());
 }
 /** 乐观锁失败应保留他人或更新后的客户资料。 */
 @Test void customerUpdateRejectsStaleVersion(){
  ...
  verify(mapper,never()).insert(anyLong(),anyString(),any());
 }
}
