...
// 根据唯一模型资源机械生成 SQL；只更新本次新增迁移及基线的生成区。
...
const statements = ['-- Generated from fcc-common/src/main/resources/flows/system-models.json.',
  '-- Execute after 20260921_staged_flows.sql. Preserve old versions and call snapshots.', 'START TRANSACTION;'];
for (const [index, [key, model]] of Object.entries(models).entries()) {
  ...
  const version = key === 'PHONE_BINDING' ? 1 : 2;
  const json = JSON.stringify(model);
  const checksum = createHash('sha256').update(json).digest('hex');
  statements.push(`INSERT INTO fcc_flow_definition(id,tenant_id,flow_key,flow_name,model_type,status,current_version)
SELECT ${definitionId},0,'SYSTEM_${key}',${literal(names[key])},'${key}','PUBLISHED',${version}
...
UPDATE fcc_flow_definition_version SET publish_status='ARCHIVED' WHERE flow_definition_id=${definitionId} AND version_no<${version} AND publish_status='PUBLISHED';
UPDATE fcc_flow_definition SET current_version=${version},status='PUBLISHED' WHERE id=${definitionId} AND current_version<${version};`);
}
statements.push('COMMIT;');
const sql = statements.join('\n\n') + '\n';
writeFileSync(new URL('docs/migrations/20260922_complete_system_models.sql', root), sql);
