...
Agent 管理用于维护 AI 模型和学习模板。模型列表展示供应商、模型明细、Base URL、优先级、启用状态和默认状态；学习 Agent 区域用于选择 Agent、模板、默认发音和句子朗读音色/语速；模板编辑区支持查看和修改模板完整内容。
...
在“个人信息 -> Agent 管理”中，可以选择学习 Agent、学习模板、默认发音、句子音色、句子语速和句子音调。登录后，句子朗读设置会保存到账户的服务端偏好中，换浏览器或清空浏览器数据后重新登录仍会同步回来；浏览器本地数据只作为临时兜底。切换学习 Agent 或模板时，系统会弹出二次确认，避免未保存的模板内容被覆盖。
