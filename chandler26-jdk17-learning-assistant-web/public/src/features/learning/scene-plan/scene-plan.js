...
import { createSceneAudioController } from '/src/features/learning/scene-plan/scene-audio-controller.js'
...
export function createScenePlanFeature(ctx) {
  ...
  function clearSceneData() {
    sceneAudioController?.clear?.()
    sceneActions?.stopAsyncTasks?.()
    api.cancelAll()
  ...
  function renderCurrentScene() {
    ...
    sceneAudioController?.stopIfDifferent(unit.id)
    sceneAudioController?.updateButton(unit.id)
    sceneAudioController?.syncStatus(unit.id)
  }
